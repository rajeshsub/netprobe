...
describe('runWithFallback', () => {
  it('returns result from the first provider when it succeeds', async () => {
    const p1 = vi.fn<[ProviderCallbacks], Promise<ProviderResult>>().mockResolvedValue(okResult)
    const p2 = vi.fn<[ProviderCallbacks], Promise<ProviderResult>>().mockResolvedValue({ ...okResult, downloadMbps: 1 })
  ...
  it('falls through to the next provider when the first throws', async () => {
    const p1 = makeProvider(async () => { throw new Error('rate limited') })
    const p2 = vi.fn<[ProviderCallbacks], Promise<ProviderResult>>().mockResolvedValue(okResult)
    const result = await runWithFallback([p1, p2], {})
  ...
  it('tries all providers in order before giving up', async () => {
    ...
    const providers: ProviderFn[] = [0, 1, 2].map(i =>
      makeProvider(async () => { order.push(i); throw new Error(`provider ${i} failed`) })
    )
  ...
  it('throws with the last error when all providers fail', async () => {
    const p1 = makeProvider(async () => { throw new Error('first failed') })
    const p2 = makeProvider(async () => { throw new Error('last failed') })
  ...
  it('calls onProviderSwitch with the new provider name and attempt number', async () => {
    const p1 = makeProvider(async () => { throw new Error('fail') })
    const p2 = vi.fn<[ProviderCallbacks], Promise<ProviderResult>>().mockResolvedValue(okResult)
