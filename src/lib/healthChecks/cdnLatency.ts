...
const CDN_TARGETS = [
  { name: 'Cloudflare', urls: ['https://cloudflare.com/cdn-cgi/trace', 'https://1.1.1.1/cdn-cgi/trace'] },
  ...
  { name: 'CloudFront', urls: ['https://d3c33hcgiwev3.cloudfront.net/', 'https://cloudfront.net/'] },
]
...
export async function measureCDNLatency(): Promise<CdnLatency[] | null> {
  const results = await Promise.all(
    CDN_TARGETS.map(t => measureCdn(t.name, t.urls))
  )
